...
```bash
python3 .Codex/skills/what-to-test/next-id.py 3   # prints the next 3 codes, advances the counter
python3 .Codex/skills/what-to-test/next-id.py --peek   # next code without advancing
```
...
2b. **Validate + repair item codes before merging.** For every item read from the existing
   ...
   `python3 .Codex/skills/what-to-test/next-id.py` and replace the malformed one, carrying
...
1. **Triage first.** Use the `triage-screenshot` skill (same repo,
   `.Codex/skills/triage-screenshot/`) to move the new file(s) out of the game's flat
