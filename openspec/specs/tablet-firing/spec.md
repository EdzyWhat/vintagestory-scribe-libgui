# tablet-firing Specification

## Purpose
The clay tablet's terminal firing step: an unfired clay tablet (wet or hard) fires in a
firepit into the permanent, read-only `-fired` variant of the same clay color, carrying its
document through the smelt. Fired tablets are immutable and never rehydrate. Wax and
already-fired tablets do not fire.
## Requirements
