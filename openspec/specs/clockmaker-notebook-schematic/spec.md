# clockmaker-notebook-schematic Specification

## Purpose
A reusable **Clockmaker's Notebook Schematic** item (`scribe:clockmakerschematic`) that grants
a trait-free grid-recipe path to the Clockmaker's Notebook, sitting alongside the existing
`tinkerer`-gated recipe. The schematic is a Glider-Schematic-style blueprint (not consumed on
craft), documented in the handbook, and occasionally sold by Commodities and Treasure Hunter
traders.
## Requirements
