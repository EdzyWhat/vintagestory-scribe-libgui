...
Collapse the local verification loop for Scribe into a single build-test-restage entry point
and an opt-in pre-push gate, so the full check runs as one invocation and pushes to `main`
are protected by the local suites.
