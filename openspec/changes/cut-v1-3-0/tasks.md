...
- [ ] 2.5 Leave `pt-br.json` untouched (English fallback). Restage (`build/restage.sh`) so the lang reload is in the staged mod.
...
- [ ] 6.1 In-game smoke (restaged 1.3.0): Getting Started lists Chalkboard + Crafting Tasks; task-types explainer describes Craft; Chalkboard handbook entry still uniqueness-first. (Author: glance Craft-from-handbook, chalkboard place/open, wet→hard→fired tablet, Transcribe stamp+sound — not a full TESTING.md sweep.)
