...
- [ ] 1.1 Re-read the editor's collapse choreography as the reference implementation:
...
- [ ] 1.2 Mine `git show refactor-reconciling-gui-rebuild:src/Mod/ScribeListView.cs` (abandoned
...
- [ ] 1.3 Confirm build + tests green on the current branch before starting (0 errors; Core suite
      passes) so any later failure is attributable to this change.

## 2. Build the ScribeAnimatedList container

- [ ] 2.1 Create `src/Mod/ScribeAnimatedList.cs` — a `StatefulWidget` taking an ordered
...
- [ ] 2.2 Implement the departure diff: on rebuild, ids present-last-frame-absent-now become
...
- [ ] 2.3 Implement collapse-end cleanup internally (drop the ghost from the cache on `onEnd`, no
      host-visible cleanup flag).
- [ ] 2.4 Implement the reappear-mid-collapse revival (D-risk / spec): an id that returns before its
      collapse ends cancels the departure and renders one live row — mirror the HUD's
      `ReconcileDeparting`.
- [ ] 2.5 Preserve slot order for simultaneous departures (reproduce the editor's display-index math
      when splicing multiple ghosts).
- [ ] 2.6 Implement the `Immediate` removal policy fully; stub/guard the `Delayed` policy path so the
...
- [ ] 2.7 Move scroll-pin-during-collapse and the hover-refresh latch inside the container, driven by
...
- [ ] 3.1 Route `ScribePinnedContent`'s row `Column` through `ScribeAnimatedList` (Immediate policy),
      keyed by `ValueKey<Guid>(TaskId)`, feeding it the existing `ScribePinRow` builder.
- [ ] 3.2 Verify the pin remove handlers (`OnPinDeleteTask`/`OnPinUnpinTask`/`OnPinCompleteTask` in
...
- [ ] 3.3 Confirm the Pinned tab still reconciles correctly through `RebuildBody()` (the container sits
      inside the persistent `ScribeDialogBody` root) and that caret/scroll/hover behavior from the
      §4.3 reconcile work is preserved.
- [ ] 3.4 Confirm Pinned completion semantics are unchanged (still immediate, no undo delay) — only the
      visual removal is now animated.

## 4. Verify (do not skip — this is the proof gate)

- [ ] 4.1 Build clean (0 errors, no new warnings); Core suite green (`dotnet test`); restage Debug.
- [ ] 4.2 Add/extend Core.Tests where the diff logic is testable game-API-free (identity diff →
...
- [ ] 4.3 Manually test in-game (Notebook Pin Tab): complete / unpin / delete a pin — confirm the row
...
- [ ] 4.4 Manually test in-game the edge cases the design flags: remove the bottom row while scrolled
      ...
      complete a row so another slides under a still cursor (its controls appear).
- [ ] 4.5 Regression-check the surfaces this change did NOT touch: editor delete, Tablet delete, and
      the HUD (fade + undo window + sink) all still behave exactly as before.

## 5. Wrap-up

- [ ] 5.1 `openspec validate extract-animated-task-list --strict` passes.
- [ ] 5.2 Record playtest verdicts (regenerate TESTING.md via the what-to-test skill).
- [ ] 5.3 Update `VSAPI-NOTES.md`/`docs/animation-lessons-learned.md` with the diffing-container
      pattern and the scroll-pin-autonomy resolution, so the follow-up (editor/HUD migration) has the
      seam documented.
- [ ] 5.4 Note the follow-up change scope: migrate editor + HUD onto `ScribeAnimatedList` (collapsing
      the duplicated choreography; wire the Delayed policy for the HUD) — a separate proposal, gated on
      this one passing its playtest.
