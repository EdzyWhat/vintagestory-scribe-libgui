...
- Keep the existing non-PvP death path unchanged (mob/environment deaths still use vanilla
  `deathmsg-<cause>-<N>` reconstruction).
...
- `notebook-history`: the Death and PvpKill requirements are refined so that (a) attacker
  resolution uses cause-entity semantics that cover melee, and (b) a PvP death's recorded
  message attributes the killer with a weapon-aware, flavored verb (tiered: weapon category →
  damage type → generic pool) rather than falling back to an unattributed "died" string.

## Impact

- Code: `src/Mod/ScribeModSystem.cs` — `OnEntityDeath` (PvpKill block) and `BuildDeathMessage`
  (attacker lookup + tiered PvP verb resolution + no-repeat generic pool cursor).
- Assets: `src/Mod/assets/scribe/lang/en.json` — new PvP verb keys (per-`EnumTool` category,
  per-`EnumDamageType`, generic pool) and the victim+killer message template.
- No Core changes (event kinds `Death`/`PvpKill` already exist); no new dependencies; no
  save-format change. Unblocks `notebook-history-tab` playtest tasks 9.4 and 9.5.
