...
- [ ] 5.1 Single-player: perform a Transcribe copy → the stamp sound plays once, exactly as the
...
- [ ] 5.2 Volume (calibration RESOLVED 2026-08-20 → the "alarm-140" level, baked into `stamp.ogg` at
...
- [ ] 5.3 Multiplayer: two clients on one shared Scriptorium. Watcher on the same
...
- [ ] 6.1 If the `SoundParams.Volume` `[0,1]` clamp vs. `SetVolume` raw-to-OpenAL distinction,
