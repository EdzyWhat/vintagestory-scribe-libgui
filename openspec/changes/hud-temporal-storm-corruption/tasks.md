...
- [ ] 1.1 Add a pure `ScribeTextCorruptor` in `src/Core` (no VS API): `Corrupt(string text, double
...
- [ ] 1.2 Unit tests (tests/Core.Tests): strength 0 returns the input unchanged; strength 1 marks
...
- [ ] 2.1 In `HudScribePins`, add a private helper that resolves
...
- [ ] 2.2 When the temporal system is absent or the setting is off, force strength 0 and
      stormActive false (graceful no-op).

## 3. HUD rendering: corruption + title swap

- [ ] 3.1 Add a lang key `scribe-hud-title-storm` = "Survive the Storm" in `lang/en.json`.
- [ ] 3.2 Thread `corruptionStrength`, `stormActive`, and `corruptionSeed` into `HudPinsContent`
      (new ctor fields, populated in `HudScribePins.Build()`), mirroring existing flags.
- [ ] 3.3 In `HudPinsContent.BuildHeader`, select `scribe-hud-title-storm` when `stormActive` else
      `scribe-hud-title`, then corrupt the chosen title string.
- [ ] 3.4 Corrupt every other rendered string — task row text, "+N more" footer, timer label and
...
- [ ] 4.1 On a recurring tick, while a trigger is active, when `capi.World.ElapsedMilliseconds`
      passes the next scheduled re-scramble, advance `corruptionSeed`, pick a new random interval in
      [2000, 8000] ms, and `ForceRebuild()`.
- [ ] 4.2 Edge-detect trigger on/off and storm active/inactive transitions to rebuild immediately
      (prompt title swap + first corruption), mirroring the existing `_stormWasActive` pattern.

## 5. Settings toggle

- [ ] 5.1 Add a client-local boolean preference (default true) for the storm-corruption effect,
      alongside the other display/behavior prefs; add label + helptext lang keys.
- [ ] 5.2 Surface it as a labeled control with helptext in the settings tab (Appearance section),
      writing through immediately.
- [ ] 5.3 `HudScribePins` reads the preference in the strength computation and rebuilds on toggle so
      enabling/disabling applies live.

## 6. Verification

- [ ] 6.1 Build passes (`build/verify.sh`); Core tests green.
...
- [ ] 6.4 In-game: confirm the re-scramble visibly shifts on a 2–8 s cadence and text layout
