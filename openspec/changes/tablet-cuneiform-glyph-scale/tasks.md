...
- [ ] 4.1 Restage per `build/restage.sh Debug` (client fully quit first — never restage while the game is
...
- [ ] 4.2 Confirm the editable field is unaffected functionally at any buffer length: type a full row's
...
- [ ] 4.3 Confirm the per-material glow (`cuneiform-contrast-glow`) still tracks the ink correctly (no
      doubled/offset halo) on at least one wet and one fired tablet view.
- [ ] 4.4 Confirm the `.cuneiform` dev harness and any non-tablet cuneiform surface are visually
      unchanged (still render at `GlyphDrawScale = 1f`).
- [ ] 4.5 Look at the resulting checkbox/icon/counter alignment against the now-shorter row and decide
...
- [ ] 5.7 Verify in-game (folds into section 4's restage): caret renders visibly shorter than the full
