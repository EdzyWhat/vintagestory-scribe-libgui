...
- [ ] 1.3 Manually verify in a local game session: put a Notebook in the crafting grid, trigger a death or storm, confirm a history entry is now recorded
- [ ] 1.4 Manually verify in a local game session: open an unrelated chest containing a Notebook (dialog open, not carried), trigger a death or storm, confirm NO history entry is recorded on that chest's notebook (regression check against the transiently-opened-container risk)
...
- [ ] 4.1 Manual/local verification: carry a chest with a Notebook inside, trigger a death and a storm, retrieve the notebook afterward and confirm both history entries appear
- [ ] 4.2 Manual/local verification: with CarryOn NOT installed, confirm history recording behaves exactly as before this task group, with no error or delay
...
Tasks 1.3, 1.4, 4.1, 4.2 are manual in-game verifications that couldn't be done headlessly (1.3/1.4
need a running client; 4.1/4.2 additionally need CarryOn actually installed). The build is restaged
(Debug) and ready for a playtest pass covering all four. If this change is archived before that
playtest happens, write these four into TESTING.md as **Backlogged** first.
