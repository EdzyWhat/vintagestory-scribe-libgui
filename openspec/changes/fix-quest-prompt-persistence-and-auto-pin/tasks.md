...
- [ ] 1.1 Add a `ScribeQuestDecision` enum (`Accepted`, `Dismissed`) and a small
...
- [ ] 1.2 Add `AutoPinOnQuestAccept` (bool, default `true`) to `ScribePlayerSettings.cs`; verify a
...
- [ ] 2.1 Create `ScribeQuestDecisionStore` (Mod-side, mirroring `ScribePinStore`'s shape) with
...
- [ ] 2.2 Wire load/save into `ScribeModSystem.ServerLifecycle.cs` alongside `pinStore`/
...
- [ ] 2.3 Add sync-to-owning-player delivery (mirroring `PushPinsTo`) sent on player join and
...
- [ ] 3.1 Add `ScribeDismissQuestPromptMessage` (Source, QuestCode, IsCompletion) and register its
      channel handler; verify the message round-trips in a local client/server smoke test.
- [ ] 3.2 In `OnServerReceivedAutoLinkQuest` (`ScribeModSystem.Quest.cs`), after a successful
...
- [ ] 3.3 In the completion-request handler path (`ScribeCompleteTaskMessage`'s server handling),
...
- [ ] 3.4 Handle `ScribeDismissQuestPromptMessage` server-side by recording a `Dismissed` decision
      for the given key; verify via local playtest that dismissing an accept-prompt, then relogging,
      does not re-raise it.
- [ ] 3.5 Send `ScribeDismissQuestPromptMessage` from `DismissQuestPrompt`
...
- [ ] 4.1 Cache the client's synced decision set (from task 2.3's delivery) on `ScribeModSystem`;
      verify it updates in place on each re-delivery without requiring a relog.
- [ ] 4.2 In `OnQuestAccepted`/`OnQuestCompleted` and `QueuePrompt`, skip queuing (and skip the
...
- [ ] 4.3 Implement Decision 4's approximate reset-clear: when `ScribeQuestWatcher` observes a
...
- [ ] 5.1 Add `AutoPin` (bool) to `ScribeAutoLinkQuestMessage`, set by the client from
...
- [ ] 5.2 In `OnServerReceivedAutoLinkQuest`, after `doc.AddQuestLink`/objective reconciliation and
...
- [ ] 5.3 Verify via local playtest that accepting with the setting off links the task but does not
      pin it (today's behavior preserved).

## 6. Settings UI

- [ ] 6.1 Add the `AutoPinOnQuestAccept` row to `ScribeSettingsContent.cs` next to the existing
...
- [ ] 7.1 Run the `Core` test suite (`dotnet test`) and confirm all new and existing tests pass.
- [ ] 7.2 Run a full local Atlas/manual pass covering: accept-then-relog (no re-prompt), dismiss-
