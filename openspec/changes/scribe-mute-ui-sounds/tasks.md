...
- [ ] 1.1 In `src/Core/ScribePlayerSettings.cs`, add a `bool MuteUiSounds` property defaulting to
  ...
  `PixelArtDisplay` one.
- [ ] 1.2 Add a Core unit test in `tests/Core.Tests` asserting the new bool defaults to `false` and
  round-trips through serialization / `Normalized()` unchanged (mirror an existing bool-preference
  test if one exists).

## 2. Silent sound player + injection

- [ ] 2.1 Add a Scribe-side `SilentSoundPlayer : ISoundPlayer` in `src/Mod/` implementing the
  ...
  `null`).
- [ ] 2.2 In each Scribe `GuiBase` dialog (`GuiDialogScribeLecternLibGui`, `ScribeSettingsDialog`),
...
- [ ] 2.3 Ensure a live toggle re-applies to an already-open dialog: hook the swap through the
...
- [ ] 3.1 In `src/Mod/ScribeSettingsContent.cs`, add a `HuggingCheckbox` for `MuteUiSounds` in the
  **Mod Behavior** section, reading `settings.MuteUiSounds` and writing via
  `onMutate(s => s.MuteUiSounds = v)` (i.e. `UpdateMySettings`).
- [ ] 3.2 Lay it out as a second column paired beside the existing "Collapse the HUD" checkbox on
  one row, reusing the existing `PairedControls`/two-column grouping helper used elsewhere in the
  form.
- [ ] 3.3 In `src/Mod/assets/scribe/lang/en.json`, add the localized label and helptext keys for the
...
- [ ] 4.1 `dotnet build src/Mod/Mod.csproj --nologo` clean; run
  `dotnet test tests/Core.Tests/Core.Tests.csproj` green.
- [ ] 4.2 Restage (`bash build/restage.sh Debug`) and fully relaunch the client.
...
- [ ] 4.6 Update `TESTING.md` with the new in-game items.
