...
- [ ] 1.1 Add affordance-styling knobs, all with sensible defaults (flat, hand-editable JSON):
...
- [ ] 2.1 In `ScribeRowElement.cs`, confirm/expose the `RoundedRect` helper and the `DrawIcon`-into-
...
- [ ] 2.2 In `ScribeBlockRowCell.cs`, evolve `ScribeHoverIconButton` into a self-drawing button:
...
- [ ] 2.3 In `RenderInteractiveElements`, keep the existing `HoverRegion` early-out, then blit the own
...
- [ ] 3.1 In `ScribeRowElement.cs`, center single-line content in the floored row height: compute one
...
- [ ] 3.2 In `GuiDialogScribeLectern.cs`, set the floating input's bounds height to
...
- [ ] 4.1 In `RowTextLayout.For`, set `TextWidth = rowWidth - textX` (stop subtracting the pin/delete
      gutters) while keeping `PinX`/`DeleteX` right-anchored as overlay anchors.
- [ ] 4.2 Relocate the drag column to the far left: `dragColWidth = DragHandleWidth * TextSizeScale`
...
- [ ] 5.1 Add `ScribeRowElement.SingleLineRowHeightFixed(capi, block, font, config, reserveAffordances)`
      built on the existing `MeasureWrappedTextHeightFixed` primitive (floored at `MinRowHeight`), giving
      a one-line row height.
- [ ] 5.2 In `GuiDialogScribeLectern.cs`, set the pin/delete/grip button bounds height to the single-line
...
- [ ] 5.3 In `ScribeRowElement.cs`, fix `IsInIconGutter` to yield only when `args.X >= Bounds.absX +
...
- [ ] 6.1 `dotnet build src/Mod/Mod.csproj -c Release` — clean. `dotnet test tests/Core.Tests` — green
...
- [ ] 7.1 If any non-obvious `GuiElementToggleButton`/`DrawIcon` behavior was relied on (e.g. the base's
