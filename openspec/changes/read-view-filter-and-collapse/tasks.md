...
- [ ] 3.2 Build the filter-pill row widget (reusing the Assignment Inbox's `BuildFilterChip`
...
- [ ] 3.3 Wire pill selection to re-filter the rendered row list using §2's matching function.
...
- [ ] 4.3 On dialog open, read the persisted pill (defaulting to All when absent) instead of the
...
- [ ] 5.2 Add the collapse-toggle control to any Read View row with a non-empty owned run, gated on
...
- [ ] 5.3 Wire the toggle to hide/show its owned run's rows, preserving order. Verify in-game:
...
- [x] 6.1 Implement group-level filter visibility: a group renders if any member (parent or child)
      ...
      members" scenarios.
- [ ] 6.2 Implement per-row shadow opacity (~50%) for a visible group's non-matching members,
...
- [ ] 6.3 Ensure a collapsed group's parent still surfaces (at its own normal-or-shadow opacity)
...
- [ ] 6.4 Ensure the All pill applies no shadow opacity to any row. Verify in-game.
...
- [ ] 7.2 Verify in-game: collapsing a group, closing, and reopening the document resumes that
...
- [ ] 8.1 Manually verify both features end-to-end on every supporting surface (Lectern, Notebook,
