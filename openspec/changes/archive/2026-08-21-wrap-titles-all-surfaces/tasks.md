...
- [x] 4.1 Verify the readable `RichText` two-line height fits within `contentBoxH` (which reserves
...
- [x] 6.1 **Lectern** — set a very long title; verify the resting title wraps to two lines, chrome
  stays clear, and a title longer than two lines ends with an ellipsis on line 2.
  - Confirmed 2026-08-21 (playtest verdict on file in TESTING.md).
- [x] 6.2 **Notebook** (and Clockmaker's Notebook) — same long-title check; verify two-line wrap and
  ellipsis-on-line-2.
  - Confirmed 2026-08-21 (playtest verdict on file in TESTING.md).
- [x] 6.3 **Scriptorium** — same long-title check; verify two-line wrap and ellipsis-on-line-2.
  - Confirmed 2026-08-21 (playtest verdict on file in TESTING.md).
- [x] 6.4 **Chalkboard** — same long-title check; verify two-line wrap fits within its taller band and
  chrome stays clear.
  - Confirmed 2026-08-21 (playtest verdict on file in TESTING.md).
- [x] 6.5 **Tablet, cuneiform ON** — regression check that the proven cuneiform two-line wrap is
  unchanged (wraps, clips at line 2, no ellipsis glyph).
  - Confirmed 2026-08-21 (playtest verdict on file in TESTING.md).
- [x] 6.6 **Tablet, cuneiform OFF** — set a long title; verify the readable RichText now wraps to two
  lines (previously single-line ellipsized), on both clay and wax materials.
  - Confirmed 2026-08-21 (playtest verdict on file in TESTING.md).
- [x] 6.7 **Editing title (readable path)** — on Lectern/Notebook/Scriptorium, verify the editing field
  stays single-line while typing, Enter commits with no newline, and the resting title re-wraps to two
  lines on commit.
  - Confirmed 2026-08-21 (playtest verdict on file in TESTING.md).
- [x] 6.8 **Short-title regression** — on every surface, confirm a title that fits on one line renders
  identically to before (no band-height change, no layout shift).
  - Confirmed 2026-08-21 (playtest verdict on file in TESTING.md).
- [x] 6.9 **Width tuning (if a value was set in 3.3)** — verify the tablet title wraps at the expected
