...
- [x] 6.1 In-game: open a lantern's Handbook page → an "Add Crafting Task" link is present (was absent).
      Click it → a Craft parent + ingredient subtasks appear.
  - Confirmed 2026-08-21 (playtest verdict on file in TESTING.md).
- [x] 6.2 In-game: create a Link and a Tracker from the "Copper Lantern" Handbook page → both rows read
      "Copper Lantern" (not `Game:Block-Lantern-Small-up`) and show the correct icon.
  - Confirmed 2026-08-21 (playtest verdict on file in TESTING.md).
- [x] 6.3 In-game: click the Link's/Tracker's label → the Handbook opens on the copper-lantern variant
      page (not a generic/empty page).
  - Confirmed 2026-08-21 (playtest verdict on file in TESTING.md).
- [x] 6.4 In-game: create tasks for a meal (an `IHandBookPageCodeProvider` item) → name resolves and the
      label opens the correct meal page.
  - Confirmed 2026-08-21 (playtest verdict on file in TESTING.md).
- [x] 6.5 In-game: with a copper-lantern Tracker, carry both copper and iron lanterns → only the copper
      lanterns count toward progress.
  - Confirmed 2026-08-21 (playtest verdict on file in TESTING.md).
- [x] 6.6 In-game: open a document saved before this change whose Tracker/Link targets are bare codes →
      every target still resolves, renders, and opens its page (backward-compat / no migration).
  - Confirmed 2026-08-21 (playtest verdict on file in TESTING.md).
- [x] 6.7 In-game: create a Tracker/Link for a plain code item (e.g. `game:ingot-copper`) → behavior and
      stored target are unchanged from before (bare code, not `stack@`).
  - Confirmed 2026-08-21 (playtest verdict on file in TESTING.md).
- [x] 6.8 Record verdicts in `TESTING.md` via the what-to-test skill.
