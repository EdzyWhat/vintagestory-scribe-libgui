...
- [x] 8.3 Manual playtest: accept a Progression Framework quest with multiple objectives (e.g. a
      Seafarer delivery quest) via a Quest Link — confirm objective subtasks appear at depth 1 with
      correct target counts.
- [x] 8.4 Manual playtest: deliver toward a linked quest's objective — confirm the subtask's live
...
- [x] 8.5 Manual playtest: confirm carrying (but not delivering) the objective's item does NOT change
      the subtask's displayed count (the carried-inventory engine must not touch it).
- [x] 8.6 Manual playtest: a non-item objective (e.g. a kill-count) shows a generic icon + readable
      label with a correct live count.
- [x] 8.7 Manual playtest: delete one objective subtask, then trigger further progress on that same
      objective — confirm it is not recreated.
- [x] 8.8 Regression check: an ordinary Tracker and a Craft task's ingredient subtasks still count
