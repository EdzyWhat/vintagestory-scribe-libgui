...
- [x] 4.1 Manual playtest: send a notice, confirm it appears in Sent History immediately as
      "Sent"; confirm the Assignee's Inbox shows nothing until the notice is actually in their
      inventory, then shows "Received."
- [x] 4.2 Manual playtest: Accept and Decline a received notice each behave exactly like an
      in-range assignment's Accept/Decline (same Sent History end-state, same no-active-notification
      behavior for Decline).
- [x] 4.3 Manual playtest: open the Accept dialog and confirm the custom title bar drags/closes
