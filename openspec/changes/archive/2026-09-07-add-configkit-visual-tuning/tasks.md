...
- [x] 3.1 Add `assets/scribe/config/configlib-patches.json` declaring all 8 settings (grouped by
  ...
  property names.
- [x] 3.2 With no config-library mod installed, confirm in-game that the mod behaves identically
...
- [x] 4.1 Install ConfigKit locally (already unpacked at
  ...
  integer setting).
- [x] 4.2 Change one integer setting (e.g. `DetectionRadius` or `BrightnessSteps`) and one float
