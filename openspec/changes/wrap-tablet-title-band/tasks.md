...
- [ ] 4.1 In-game: on a wet tablet, set a very long title (e.g. a long item name pasted in) → the resting
...
- [ ] 4.2 In-game: click the pencil and type a long title past one line → the editing field wraps to a
      second line (not clipped off the right), and Enter commits it, showing the same two-line resting title.
- [ ] 4.3 In-game: confirm a SHORT tablet title looks identical to before (single line, no band growth), and
      that Lectern/Notebook/Scriptorium titles are visually unchanged (still single-line with ellipsis).
- [ ] 4.4 In-game: confirm the disable-cuneiform fallback still renders a readable single-line title on the
...
- [ ] 6.1 If the two-line clip budget or the tablet-only band-height override lands as a non-obvious layout
