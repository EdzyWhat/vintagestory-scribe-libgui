...
- [ ] 1.1 Create `.claude/skills/reconcile-playtest/reconcile.py` (stdlib only). Add `--dry-run` and
      `--archive` flags and a repo-root resolver (walk up for `TESTING.md`, mirroring `server.py`'s
      `find_testing_file_upward`).
- [ ] 1.2 Load pending `.playtest-submissions/*.json` (top-level files only, not `reviewed/`). Parse
...
- [ ] 1.3 Build an index of `TESTING.md` items keyed by fingerprint, and a resolver from `taskId`
...
- [ ] 2.1 For `verdict: "pass"`: write a `- **Confirmed <date>** (submission <ts>): <note>` line under
...
- [ ] 2.2 For `verdict: "fail"`: write a `- **Still broken <date>:** <note>` line under the TESTING.md
      item; leave the tasks.md box `[ ]`.
- [ ] 2.3 Idempotency: never insert a duplicate verdict line, never re-flip an already-checked box,
...
- [ ] 2.4 Pass `date`/timestamp values in from the CLI/runner (do NOT read a wall clock inside logic
      that must stay testable) so fixture runs are deterministic.

## 3. Triage inbox

- [ ] 3.1 For `verdict: null` items and every non-empty `generalNotes`, append an entry to
...
- [ ] 3.2 De-dupe triage entries across re-runs (key on submission ts + fingerprint / a `generalNotes`
      hash) so the inbox is append-safe.

## 4. Ready-to-archive detection + opt-in archive

- [ ] 4.1 After propagation, scan in-progress changes (`openspec list --json`) and mark those whose
      tasks.md is now fully checked as ready-to-archive.
- [ ] 4.2 With `--archive`, run `openspec archive <name> -y` for each, serialized, capturing per-change
      success/collision into the report; without the flag, only list them.

## 5. Report + submission lifecycle

- [ ] 5.1 Emit a machine-readable JSON report (`{applied, readyToArchive, triage, errors}`) and a
      concise human summary to stdout. `--dry-run` produces the report with zero writes.
- [ ] 5.2 Move a submission to `reviewed/` only when all its items reached a terminal write AND its
      generalNotes were captured to triage; otherwise leave it pending and list it under triage/errors.
- [ ] 5.3 Flag `pass` items whose `note` contains caveat markers ("but", "doesn't", "except", "todo",
      case-insensitive) in the report's summary as "glance" items — without blocking their auto-apply.

## 6. Tests (deterministic core)

- [ ] 6.1 Add fixture submissions (a clean pass, a fail, a null-with-note, a partial, a multi-submission
...
- [ ] 6.2 Regression: dry-run over the existing 95 `reviewed/` submissions and assert the derived
...
- [ ] 7.1 Write `.claude/skills/reconcile-playtest/SKILL.md`: run `reconcile.py`, read its JSON report,
...
- [ ] 7.2 Cross-link from the `what-to-test` skill and `TESTING.md` preamble: the loop is
...
- [ ] 8.1 Update the reconcile-playtest README/usage and note the optional (out-of-scope) follow-up in
      `vs-playtest-checklist` (always-capture verdict state; dedicated new-feedback field).
- [ ] 8.2 `openspec validate add-playtest-reconcile-loop --strict` passes.
- [ ] 8.3 Dogfood once: run `reconcile-playtest` against a real pending submission (or a copied fixture)
