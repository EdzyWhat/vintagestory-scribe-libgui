...
- [ ] 6.1 Open an ink-and-quill (or similar liquid recipe) Handbook page, Add Crafting Task, and confirm the
...
- [ ] 6.2 With an empty inventory the liquid Tracker reads `0 / N`. Pick up a bucket/bowl holding the liquid;
      the count rises to the container's litres within ~1 s (event path + poll backstop).
- [ ] 6.3 Carry multiple containers of the same liquid and confirm the litres sum; a container of a different
      liquid contributes 0.
- [ ] 6.4 Carrying ≥ the target litres marks the Tracker satisfied and applies the player's tracker-completion
      setting (complete / delete / nothing); dropping below the target afterward does not re-fire.
- [ ] 6.5 The liquid Tracker also updates correctly when pinned to the HUD (same `CountCarried` path).
- [ ] 6.6 Sweep a few real recipes with `.scribeprobe` (Open Question in design.md): confirm any block-`type`
...
- [ ] 6.7 Record verdicts in TESTING.md per the project's playtest workflow.
