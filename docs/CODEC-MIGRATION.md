...
| `HistoryStore` | binary (item attribute) | v3 | v1–v3 (progressive reads) | `ApplyV2ToV3Migrations` — assigns a synthetic negative-sequence `InGameTimestamp` (preserving existing order) to any v1/v2 payload; v2's `EntryId` then reads behind `version >= 2` |
