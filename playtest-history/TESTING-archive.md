...
> from both. Archived 2026-09-06 as `2026-09-06-read-view-filter-and-collapse`; two
> follow-up bug reports from this playtest became `read-view-collapse-affordance-fixes`.

- [x] `000000a0` **Test filter pills.** Open a supporting surface (Lectern/Notebook/etc.) — confirm
      ...
      - **Confirmed 2026-09-06** (submission 2026-09-06T16-48-19): "(no note)"
- [x] `000000a1` **Test filter persistence.** On one instance, select a non-All pill, close the
      ...
      *(read-view-filter-and-collapse 4.3)*
      - **Confirmed 2026-09-06** (submission 2026-09-06T16-48-19): "(no note)"
- [x] `000000a2` **Test collapse toggle.** Confirm the collapse toggle appears only on a Quest Link
      ...
      - **Confirmed 2026-09-06** (submission 2026-09-06T16-48-19): "(no note)"
- [x] `000000a3` **Test shadow opacity.** Under a non-All filter, on a group where only some members
      ...
      - **Confirmed 2026-09-06** (submission 2026-09-06T16-48-19): "(no note)"
- [x] `000000a4` **Test collapsed group surfaces via hidden match.** Collapse a group where one of
      ...
      *(read-view-filter-and-collapse 6.3)*
      - **Confirmed 2026-09-06** (submission 2026-09-06T16-48-19): "(no note)"
- [x] `000000a5` **Test collapse persistence.** Collapse one group (leave others expanded), close and
      ...
      - **Confirmed 2026-09-06** (submission 2026-09-06T16-48-19): "(no note)"
- [x] `000000a6` **Test every surface + Tablet exclusion.** Confirm both the pill row and collapse
...
- [x] `00000016` **Fired matches wet.** Repeat the link + body checks on a **fired/hardened** (read-only)
      ...
      - **Fix landed 2026-08-21, ready to retest:** the state-aware glow + per-state ink fix shipped.
        `add-tablet-state-glow-modifier` (light halo on hard/fired) landed first, then was superseded by
        `adopt-glyph-forge-tablet-themes`, which bakes the glyph-forge per-`(material, state)` readability
        bundle: per-state body + link ink, a stroke-weight scale (0.9 wet → 1.1 fired), and a seated-drop
        LIGHT halo on hard/fired (lifts dark ink off the darker backdrop). Built + restaged (Debug).
        Retest via `/scribe tablet hard|fired` on blue/red/fire — fired/hardened cuneiform should now read
        as legibly as wet, colors matching the wet palette. Tracked under
        `adopt-glyph-forge-tablet-themes` §9.1/§9.5 (`0000001f` covers the fuller per-state sweep).
      - **Confirmed 2026-08-21** (submission 2026-08-21T11-24-55): "pass." Fired/hardened cuneiform now
        reads as legibly as wet — the state-aware light halo lifts the darker fired ink off the darker
        backdrop, colors match the wet palette. Closes the long-standing 8/20 fired/hardened gap. (A
        subsequent glyph-forge retune landed the same session — see `00000023` for the re-glance.)
...
## manage-terminal-assignment-records

> Delete control on terminal-state rows (Inbox + Sent History, caret-expand-gated,
> "Remove Terminal Record") + a title-bar expand/collapse-all toggle for both tabs.
> Archived 2026-09-02 as `2026-09-02-manage-terminal-assignment-records`. Superseded in part
> by `split-assignment-delete-by-viewer` (per-side delete scoping — see live TESTING.md).

- [ ] `00000073` **Delete a Completed Inbox record.** As the Assignee, expand a Completed row in
      the Inbox, tap "Remove Terminal Record" — confirm it disappears from the Inbox and does not
      reappear. *(manage-terminal-assignment-records 6.1)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T17-35-07): "(no note)"
- [ ] `00000074` **Delete a terminal Sent History record.** As the Assigner, expand a
      Declined/Cancelled/Discarded row in Sent History, tap "Remove Terminal Record" — confirm it
      disappears from Sent History. *(manage-terminal-assignment-records 6.2)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T17-35-07): "(no note)"
- [ ] `00000075` **Check delete control is terminal-only.** Expand an Unaccepted and an Accepted
      row — confirm neither ever shows a "Remove Terminal Record" control.
      *(manage-terminal-assignment-records 6.3)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T17-35-07): "(no note)"
- [ ] `00000076` **Check expand/collapse-all toggle.** With a mix of collapsed/expanded rows and at
      least one terminal record, tap the title-bar toggle — confirm every visible row expands, tap
      again — confirm every visible row collapses, and confirm the icon/tooltip reflects each state
      correctly. *(manage-terminal-assignment-records 6.4)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T17-35-07): "(no note)"
- [ ] `00000077` **Check toggle respects the filter.** Apply a state filter chip that hides some
      rows, then use the title-bar toggle — confirm only the currently-visible rows change
      expansion state. *(manage-terminal-assignment-records 6.5)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T17-35-07): "(no note)"
- [ ] `00000078` **Check toggle only shows on Inbox/Sent History.** Confirm the expand/collapse-all
      toggle button is absent on the Read/Editor/Pinned/Settings views and only appears on Inbox
      and Sent History. *(manage-terminal-assignment-records 6.6)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T17-35-07): "(no note)"

## capture-assignment-accept-destination

> Restage BOTH client and server together before testing (store version bump 4 → 5 — a
> mismatched pair reading/writing different versions could misparse).
> Change archived as `2026-09-02-capture-assignment-accept-destination`.

- [x] `0000007b` **Check accepted-into label on a titled item.** As the Assigner, send a
      task; as the Assignee, Accept it into a Notebook you've titled (e.g. "Book of Nick")
      — expand the row in your Inbox, confirm it reads `Accepted into Notebook "Book of
      Nick" — <date>`. *(capture-assignment-accept-destination 5.1)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T18-15-36): "Works. YOu get visibility on the Inbox, but not in the Sent Assignment History, which is the correct behavior!"
- [x] `0000007c` **Check accepted-into label falls back on an untitled item.** Accept a task
      into a Notebook that still has its default (never-renamed) title — confirm the line
      falls back to the bare item name (e.g. "Accepted into Notebook — <date>"), not the
      literal default title text. *(capture-assignment-accept-destination 5.2)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T18-15-36): "(no note)"
- [x] `0000007d` **Check pre-existing Accepted records render unchanged.** Expand an
      assignment Accepted before this update (pre-existing save data, if available) —
      confirm it still renders its Accepted line as date-only, with no error or placeholder
      text. *(capture-assignment-accept-destination 5.3)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T18-15-36): "(no note)"
- [x] `0000007e` **Check Read/Editor/Pin Tab stay date-only.** Confirm the Read/Editor-row
      tooltip and Pin Tab still show date-only for an accepted assignment (unchanged scope
      — no destination text there). *(capture-assignment-accept-destination 5.4)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T18-15-36): "(no note)"

## split-assignment-delete-by-viewer

> Restage BOTH client and server together before testing (store version bump 5 → 6 — a
> mismatched pair, or an old client omitting the new `Side` byte, could misparse).
> Change archived as `2026-09-02-split-assignment-delete-by-viewer`.

- [x] `0000007f` **Check self-assignment delete is per-side.** Send an assignment to
      yourself, bring it to a terminal state, delete it from the Inbox only — confirm it
      disappears from the Inbox but STILL shows in Sent Assignment History.
      *(split-assignment-delete-by-viewer 6.1)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T18-15-36): "(no note)"
- [x] `00000080` **Check the second delete fully removes it.** From that same still-visible
      Sent Assignment History row, delete it too — confirm it now fully disappears from
      both tabs. *(split-assignment-delete-by-viewer 6.2)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T18-15-36): "(no note)"
- [x] `00000081` **Check two-player delete independence.** With two separate players, have
      the Assigner delete a terminal record from Sent Assignment History — confirm the
      Assignee's Inbox still shows it, unaffected; then repeat with the Assignee deleting
      first — confirm the Assigner's Sent Assignment History is unaffected.
      *(split-assignment-delete-by-viewer 6.3)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T18-15-36): "(no note)"
...
> Change archived as `2026-09-02-refine-assignment-desk-inbox-ux`. Two Backlogged items
> (batch-by-session, thinner "+" icon) stayed live in TESTING.md since they're still
> unscoped — see the live file.
...
- [x] `00000049` **Check theme-picker button.** In Scribe Settings' Window Appearance
      ...
      - **Still broken 2026-09-01:** (submission 2026-09-01T13-31-56) "Take the time to use AskUserQuestion to solve this with me."
      - **Confirmed 2026-09-01**: the DrawOrder/stacking theory was disproven by a clean repro (failed
        with zero other dialogs open). Real root cause: the client log showed `noprivilege` —
        `ui`/`settings` never calls `RequiresPrivilege`, so the check fell through to
        `Caller.HasPrivilege(null)`, which only a real typed command's automatic `CallerPrivileges =
        ["*"]` grant passes. Fix: granted the same wildcard on our synthetic `Caller`. Playtest-
        confirmed working after restage.
...
- [x] `0000004b` **Check multi-item batch send.** Stage a document with at least one Task,
...
- [x] `0000004c` **Check delete-from-source.** Repeat the batch send with "Delete from
...
- [x] `0000004f` **Check reworked chip colors.** Open the Assignment Inbox or Create
...
- [ ] `00000050` **Check newest-first Inbox order.** Send a couple of assignments (or a
      ...
      - **Obsolete 2026-08-31**: root-caused (refine-assignment-desk-inbox-ux 14.3) — the previous
        ...
        see `00000056`.
- [x] `00000051` **Check Create Assignments tab split.** Open the Assignment Desk — confirm
      ...
      - **Confirmed 2026-08-31** (submission 2026-08-31T20-35-02): "(no note)"
- [x] `00000052` **Check staged-list box styling.** On the Create Assignments tab, confirm
      ...
      - **Confirmed 2026-08-31** (submission 2026-08-31T20-35-02): "(no note)"
- [x] `00000053` **Check Accept respects New Task Insert.** As the ASSIGNEE, set New Task
...
- [x] `00000054` **Check tooltip title removed.** Look at (and pick up) a placed/carried Inbox
      ...
      - **Confirmed 2026-08-31** (submission 2026-08-31T22-17-42): "(no note)"
- [x] `00000055` **Check Accept-candidate scope.** With writeable Scribe items scattered
      ...
      - **Confirmed 2026-08-31** (submission 2026-08-31T22-17-42): "(no note)"
- [x] `00000056` **Check same-day two-batch ordering.** Send two separate multi-item
...
- [x] `00000057` **Check Sent History ordering.** Open Sent Assignment History after sending
      ...
      *(refine-assignment-desk-inbox-ux 14.9)*
      - **Confirmed 2026-08-31** (submission 2026-08-31T22-17-42): "(no note)"
- [x] `00000058` **Check filter pill rework.** On either the Inbox or Sent Assignment
...
- [x] `00000059` **Check disabled Accept button styling.** Open an Inbox row with an
      ...
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `0000005a` **Check Sent History icon.** Open the Assignment Desk — confirm the Sent
      Assignment History nav button shows the same open-book glyph used for task Links
      (not the guestbook icon, not the old scroll). *(refine-assignment-desk-inbox-ux 15.3)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T17-35-07): "Correct, that is what it shows."
- [x] `0000005b` **Check per-transition history stubs.** Expand an assignment in the Inbox or
      ...
      *(refine-assignment-desk-inbox-ux 15.4)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `0000005c` **Check staging-slot styling + copy.** Open the Create Assignments tab —
      ...
      3-step numbered walkthrough. *(refine-assignment-desk-inbox-ux 15.6)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `0000005d` **Check batch-accept adjacency.** Accept a batch's parent task, then (without
      ...
      *(refine-assignment-desk-inbox-ux 15.1)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `0000005e` **Check Sent History live refresh.** As the ASSIGNER, leave Sent Assignment
      ...
      *(refine-assignment-desk-inbox-ux 15.8)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `00000060` **Check assigned-task stamp art.** Open the Inbox, Editor, and Pin Tab —
      ...
      *(refine-assignment-desk-inbox-ux 16.1)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `00000072` **Check Accept always picks when 2+ carried.** Carry two (or more) eligible
      Scribe items, one of them your most-recently-opened — open an Inbox with an unaccepted
      task and tap Accept. Confirm the picker now appears (not an immediate accept) with the
      last-opened item pre-selected; confirm choosing a different candidate and confirming
      lands the task on that one instead. *(refine-assignment-desk-inbox-ux 17.2)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T17-35-07): "Works, but it looks like on the Assignment Inbox we are missing what Scribe item the task was accepted onto. I thought we'd implemented that update already..."

## assignment-icon-and-tab-defaults

> Change archived as `2026-09-02-assignment-icon-and-tab-defaults`.

- [x] `0000006e` **Check marker position + tooltip.** As the assignee, open Read, Edit, and the Pin
      ...
      6.2)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `0000006f` **Check Scriptorium right-click default.** Right-click a placed Scriptorium: it
      ...
      6.2)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `00000070` **Check Lectern right-click default.** Right-click a placed Lectern: it should open
      ...
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `00000071` **Check crouch+right-click quick-add still works.** On both the Scriptorium and the
...
> gained `SourceIsDeskDocument`. Change archived as `2026-09-02-add-assignment-desk-own-tasks`.

- [x] `00000061` **Check empty state has no button.** With the staging slot empty and no
      ...
      *(add-assignment-desk-own-tasks 5.1)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `00000062` **Check pull-from-Desk populates list.** Author a task on the Desk's own
      ...
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `00000063` **Check staged item overrides Desk source.** With the Desk source active
      ...
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `00000064` **Check live-tracking via Editor.** With the Desk source active, switch to
      ...
      *(add-assignment-desk-own-tasks 5.4)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `00000065` **Check subtask cascade from Desk source.** Select a parent task (with
      ...
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `00000066` **Check delete-from-Desk-source symmetry.** With "Delete from source on
      ...
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `00000067` **Check Read/Editor tab parity + lock.** Confirm the Assignment Desk's Read
      ...
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `00000068` **Check no Pinned tab.** Open the Assignment Desk — confirm no Pinned nav
      button appears (six tabs: Create Assignments, Sent History, Inbox, Read, Editor,
      Settings). *(add-assignment-desk-own-tasks 5.8)*
      - **Confirmed 2026-09-01** (submission 2026-09-01T13-31-56): "(no note)"
- [x] `00000069` **Restage client + server together first.** Before any of the above:
